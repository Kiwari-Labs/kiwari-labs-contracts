...
you don't need to get to iterate or traveler over the `list` for each `Slot` to calculate the entire slot balance if the slot can presume not expire. otherwise struct `Slot` also providing vertical in a sorted list.
