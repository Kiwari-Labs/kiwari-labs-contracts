...
library SlidingWindow {
    ...
    function _calculateSlot(
        ...
        assembly {
            if gt(blockNumber, startBlockNumber) {
                blockNumber := sub(blockNumber, startBlockNumber)
                slotSize := div(blockPerEpoch, slotSize)
                result := div(mod(blockNumber, blockPerEpoch), slotSize)
            }
        }
    }
    ...
    function _calculateEpochAndSlot(
        ...
        slot = _calculateSlot(state._startBlockNumber, state._blockPerEpoch, blockNumber, state._slotSize);
    }
