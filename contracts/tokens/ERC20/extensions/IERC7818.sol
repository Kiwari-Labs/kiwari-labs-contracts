...
interface IERC7818 is IERC20 {
    ...
    /// @param id The ID of the round, period, or token.
    /// @return uint256 The balance of the specified `id`.
    function balanceOf(address account, uint256 id) external view returns (uint256);

    /// @dev Retrieves the duration (in blocks or seconds) a token remains valid.
    ...
    /// @return bool True if the transfer succeeded, false if the token is expired.
    ...
    /// @dev Retrieves unexpired token balances for a specific account.
    ///      Supports pagination to handle large data sets.
    /// @param account The address of the account.
    /// @param offset The starting index for pagination.
    /// @param limit The maximum number of items to retrieve.
    /// @return uint256[] A list of unexpired token balances.
    function tokenList(address account, uint256 offset, uint256 limit) external view returns (uint256[] memory);
}
