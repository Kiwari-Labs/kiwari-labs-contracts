...
export const run = async () => {
  describe("CalculationFrame", async function () {
    it("[HAPPY] calculate correctly frame if the current block is in the first slot period of the first epoch", async function () {
      ...
      const [fromEpoch, toEpoch, fromSlot, toSlot] = await lightWeightSlidingWindow["frame(uint256)"](blockNumber);
    ...
    //     const [fromEpoch, toEpoch, fromSlot, toSlot] = await lightWeightSlidingWindow.frame(blockNumberList[i]);
