...
export const run = async () => {
  describe("General", async function () {
    it("[HAPPY] query middle from even linked list", async function () {
      ...
      expect(await lightWeightDoublyList.middle()).to.equal(2);
    });
    ...
    it("[HAPPY] query first partition from odd linked list", async function () {
      ...
      // [1, 2, 3, 4, 5] => [1, 2]
      const list = await lightWeightDoublyList.firstPartition();
    ...
    it("[HAPPY] query second partition from odd linked list", async function () {
      ...
      // [1, 2, 3, 4, 5] => [5, 4, 3]
      const list = await lightWeightDoublyList.secondPartition();
      expect(list.length).to.equal(3);
    ...
    it("[HAPPY] query partition from index to head", async function () {
      ...
      const start = 7;
      const list = await lightWeightDoublyList.pathToHead(start);
    ...
    it("[HAPPY] query partition from index to tail", async function () {
      ...
      const start = 7;
      const len = 10;
      const list = await lightWeightDoublyList.pathToTail(start);
    ...
    it("[HAPPY] query partition", async function () {
      ...
      let start = 7;
      const len = 10;
      const list = await lightWeightDoublyList.partition(start);
    ...
    it("[HAPPY] query previous node", async function () {
      ...
      expect(await lightWeightDoublyList.previous(2)).to.equal(1);
    });
    ...
    it("[HAPPY] query next node", async function () {
      ...
      expect(await lightWeightDoublyList.next(2)).to.equal(3);
    });

    it("[UNHAPPY] always returns empty when the linked list has no element", async function () {
      const {lightWeightDoublyList} = await deployLightWeightDoublyListLibrary();
      let list = await lightWeightDoublyList.firstPartition();
      expect(list.length).to.equal(0);
      list = await lightWeightDoublyList.secondPartition();
      expect(list.length).to.equal(0);
      list = await lightWeightDoublyList.partition(0);
      expect(list.length).to.equal(0);
      list = await lightWeightDoublyList.pathToHead(0);
      expect(list.length).to.equal(0);
      list = await lightWeightDoublyList.pathToTail(0);
      expect(list.length).to.equal(0);
    });
    ...
    it("[UNHAPPY] the index not found in linked list", async function () {
      ...
      let list = await lightWeightDoublyList.partition(11);
      expect(list.length).to.equal(0);
      list = await lightWeightDoublyList.pathToHead(11);
      expect(list.length).to.equal(0);
      list = await lightWeightDoublyList.pathToTail(11);
      expect(list.length).to.equal(0);
    });

    it("[UNHAPPY] query middle from emtpy linked list", async function () {
      const {lightWeightDoublyList} = await deployLightWeightDoublyListLibrary({});
      expect(await lightWeightDoublyList.middle()).to.equal(0);
