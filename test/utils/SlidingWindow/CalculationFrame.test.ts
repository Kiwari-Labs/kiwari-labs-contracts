...
import {deploySlidingWindowLibrary} from "./utils.test";
...
export const run = async () => {
  describe("CalculationFrame", async function () {
    it("[HAPPY] calculate correctly frame if the current block is in the first slot period of the first epoch", async function () {
      ...
      const {slidingWindow} = await deploySlidingWindowLibrary({startBlockNumber, blockPeriod, slotSize, frameSize});

      const blockNumber = Math.floor(blockPeriodSlot * 0.5) + startBlockNumber;

      const [fromEpoch, toEpoch, fromSlot, toSlot] = await slidingWindow["frame(uint256)"](blockNumber);
