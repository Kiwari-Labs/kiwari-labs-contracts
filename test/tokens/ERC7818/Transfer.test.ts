...
export const run = async () => {
  describe("Transfer", async function () {
    it("[HAPPY] transfer correctly if frame size is 2 and slot per era is 4", async function () {
      ...
      const blockPerSlot = await erc7818.getBlockPerSlot();
      const blockPerFrame = await erc7818.getFrameSizeInBlockLength();
    ...
    it("[UNHAPPY] transfer from zero address", async function () {
      ...
      expect(erc7818.badTransfer(common.zeroAddress, await alice.getAddress(), 1))
        .to.be.revertedWithCustomError(erc7818, ERC20.errors.ERC20InvalidSender)
    ...
    it("[UNHAPPY] transfer to zero address", async function () {
      ...
      expect(erc7818.connect(alice).transfer(common.zeroAddress, 1))
        .to.be.revertedWithCustomError(erc7818, ERC20.errors.ERC20InvalidReceiver)
        .withArgs(common.zeroAddress);
    });
    ...
    it("[UNHAPPY] insufficient balance", async function () {
      ...
      expect(erc7818.connect(alice).transfer(await bob.getAddress(), 1))
        .to.be.revertedWithCustomError(erc7818, ERC20.errors.ERC20InsufficientBalance)
